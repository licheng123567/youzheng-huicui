...
require_grep "trace:[[:space:]]*'off'" "$REPO_ROOT/frontend/playwright.uat.config.ts"
reject_grep 'retain-on-failure' "$REPO_ROOT/frontend/playwright.uat.config.ts"
...
require_grep 'expect\(repay\)\.toBeVisible\(\{ timeout: 5_000 \}\)' "$REPO_ROOT/frontend/e2e/business-lifecycle.spec.ts"
reject_grep 'test\.setTimeout\(90_000\)' "$REPO_ROOT/frontend/e2e/business-lifecycle.spec.ts"

...
清理任务每天 03:30 自动跑（错开 03:00 的备份），见 `RetentionService`。

| 数据 | 处理 | 时点 |
|---|---|---|
| 业主姓名 / 手机 / 身份证 / 通讯地址 / **房号** | 去标识化（姓名→占位、房号→`—`、手机整行删、身份证删） | 结案后 **60 天** |
| **通话录音音频 + 转写全文 + 逐句句段** | **删除**（录音无法匿名：声纹本身是敏感个人信息） | 结案后 **180 天** |
| 小区 / 项目 | **保留**（报表聚合维度，单凭它定位不到个人） | — |
| 回款 / 佣金 / 支付申请单 | **不动**（会计凭证，法定保管期远长于此） | — |

**录音为什么是 180 天而不是 60 天**：催收最大的法律风险是业主投诉「暴力催收」，
那时录音是**平台自证清白的唯一证据**。60 天就删，投诉来了手里什么都没有。

**时钟起点不是 `closed_at`**，而是 `max(结案, 末笔回款, 末次存证, 末份法务文书)` ——
坏账案件可能重启诉讼，佣金争议常在结案数月后才爆发。

**法律保留（legal hold）**：投诉/诉讼/监管检查期间，**物业**（PL/PC，权限点 `case.hold`）
可对案件置 `POST /cases/{id}/legal-hold`（reason 必填），清理一律跳过；置/撤都写审计。

调参（一般不用改）：
```
HUICUI_RETENTION_ENABLED=true          # 关掉 = 个人信息永久留存，仅供本地排查
HUICUI_RETENTION_PII_DAYS=60
HUICUI_RETENTION_RECORDING_DAYS=180
```

> ⚠️ **备份里还有原始 PII**：主库去标识化后，保留期内（默认 14 天）的备份仍是完整数据 ——
> 「删除」要到备份轮换完才真正生效。这不是 bug，是这套方案的固有边界，必须写进你的隐私政策。

---

## 四之二半、话术飞轮定时结算

每天 **04:00** 自动跑一次（错开 03:00 备份、03:30 留存清理）：把真实通话结果（承诺兑现 / 回款）
按 `promise.script_id` 的归因回流进话术库，重算每句话术的使用量 / 兑现率 / 回款率 / Wilson 下界；
顺带把**达标的 AI 变体**（默认 `uses ≥ 300` 且 `uplift ≥ 2%`、A/B 已跑赢）自动升为现行话术，
旧文本保留可回滚，每条都写审计（`actor=system`）。

纯聚合、幂等，跑几遍结果一样。两条边界是硬的：
**无归因的话术一个字段都不动**（不抹平专家录入的种子），**专家话术永不自动晋升**（那必须平台运营手点）。

话术库页面的「飞轮结算」按钮保留 —— 改了阈值、补了归因想立刻看效果时，不必等到明早。

```
HUICUI_FLYWHEEL_AUTO=true              # 关掉 = 不手点就不结算，统计一直停在上次快照
HUICUI_FLYWHEEL_CRON=0 0 4 * * *
```

晋升阈值不在这里，在后台「AI 配置」的 `flywheel.trigger`（形如 `uses>=300 AND wilson_uplift>=0.02`）。

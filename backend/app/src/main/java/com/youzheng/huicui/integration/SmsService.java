 ...
 *   验证码 sendVerificationCode → 普通短信（即时）。登录前无 org，不落 sms_record。
...
public class SmsService {
    ...
    public void sendPayLinkSms(long caseId, long orgId, long projectId, String token) {
        ...
        if (phone == null || phone.isBlank()) {
            recordSms(orgId, caseId, projectId, "缴费链接", "FAILED", "案件无主号");
            return;
        }
        try {
            ...
            recordSms(orgId, caseId, projectId, "缴费链接", "SENT", null);
        } catch (ApiException e) {
            log.warn("缴费链接短信发送失败 case={}: {}", caseId, e.getMessage());
            recordSms(orgId, caseId, projectId, "缴费链接", "FAILED", e.getMessage());
        }
    }
    ...
    public String sendVideoNotify(long orgId, Long caseId, Long projectId, String templateId,
        ...
        try {
            ...
            recordSms(orgId, caseId, projectId, "视频:" + templateId, "SENT", null);
            return taskId;
        } catch (ApiException e) {
            recordSms(orgId, caseId, projectId, "视频:" + templateId, "FAILED", e.getMessage());
            throw e;
        }
    }
    ...
    private void recordSms(Long orgId, Long caseId, Long projectId, String template, String status, String failure) {
        if (orgId == null) return;   // 无 org（如验证码）不落流水（sms_record.org_id NOT NULL）
        try {
            jdbc.update(

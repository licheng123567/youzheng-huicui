...
| T-06 | SA 平台公海再派 | 服务器回归的成功再派已完成，但断言 `.first()` 命中关闭抽屉内的隐藏说明文字；旧用例还把任意首行/首个服务商误称为特定退回案 | `TEST_BUG` | 用例依赖列表顺序并把瞬时提示与静态说明混为一谈；同一合成基线仅剩 1 个服务商容量名额，却重复要求两次成功写入 | 按种子房号 `S0-101`/`RET-101` 精确选案并等待对应 POST；分别断言 200+抽屉关闭、409+护栏提示；删除容量上不可能稳定成功的重复写入，定向复验 2/2 通过 |
...
| 部署后 HTTP 兼容生命周期 | 1/1 通过，15.2 秒 | 原失败容器来源 `http://web`；回款抽屉 5 秒硬断言、回款 `¥12.34` 与平台对账全部通过 |
| 首次部署后全量回归 | 273 通过、8 跳过、1 测试失败 | 282 项；唯一失败为 T-06 的隐藏说明定位，扫描后 UAT 重置与独立验证通过 |
| 平台公海再派定向复验 | 2/2 通过 | 干净合成基线；精确覆盖成功再派与原退回服务商 409 护栏 |

...
| T-07 | SA 批次结项预览 | 第二次部署后回归中抽屉已打开，但旧断言按抽屉后代文本等待到整条用例超时，未直接报告 `/close-preview` 的完成状态 | `TEST_OBSERVABILITY` | 用例没有把预览 GET 作为操作完成条件，也没有按抽屉可访问名称定位 | 点击前监听精确 GET 路径，10 秒内要求 200；按抽屉 accessible name 定位。干净基线定向复验 1/1 通过 |
| T-08 | VL 催收员佣金确认支付 | 页面实际已把两张单更新为 `SETTLED`，旧脚本却把仍在页面中的另一枚“确认支付”按钮误当二次确认，又发起一次写入并耗尽超时 | `TEST_BUG` | 产品页面没有二次确认框；正则 `/确定|确认/` 会命中业务表格里的下一枚写按钮 | 只提交一次，等待精确确认支付 POST=200，并按原佣金单号断言该行变为“已结算”且按钮消失；定向复验 1/1 通过 |
...
| 第二次部署后全量回归 | 270 通过、9 跳过、2 测试失败 | 281 项；产品页面结果显示佣金已正确结算，失败分别为 T-07 的预览完成条件不明确与 T-08 的重复点击；扫描后 UAT 重置与独立验证通过 |
| 批次结项、佣金支付定向复验 | 2/2 通过 | 干净合成基线；结项预览 GET=200，佣金确认支付 POST=200 且原单据行变为“已结算” |

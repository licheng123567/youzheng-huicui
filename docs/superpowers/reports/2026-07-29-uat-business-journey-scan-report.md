...
- 候选修复提交：`a105d54`（成员）、`b11ade0`（批次减免权限）、`5229d0f`（扫描与验收加固）、`6f3a7f1`/`0a015a6`（bare checkout 验收兼容）、`18106cf`（GNU/POSIX chmod 兼容）、`ed96a3f`（契约环境隔离）
...
| 部署后契约兼容 | 通过 | 服务器扫描在数据重置前发现 `chmod -x` 的 GNU/POSIX `umask` 差异及静态契约继承真实 `UAT_STATE_DIR`；改用 `chmod a-x` 并清除夹具的外部 UAT 运行时变量，遗留的同 SHA `pending-sha` 已核验后删除 |

...
- 候选修复提交：`a105d54`（成员）、`b11ade0`（批次减免权限）、`5229d0f`（扫描与验收加固）、`6f3a7f1`/`0a015a6`（bare checkout 验收兼容）、`18106cf`（GNU/POSIX chmod 兼容）
...
| 部署后契约兼容 | 通过 | 首次服务器扫描在数据重置前发现 `chmod -x` 的 GNU/POSIX `umask` 差异；改为明确的 `chmod a-x`，本地静态与全扫描契约均通过 |

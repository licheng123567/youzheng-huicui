...
- 候选修复 SHA：`ffab2891908f8dcac9f5482fda152628303bcd86`
- 口令处理：只从服务器环境文件注入进程环境；报告、命令输出和 Playwright trace 均不保存口令或令牌

## 结论

完整回归共执行 273 项：261 通过、10 跳过、2 失败。两个失败均由同一个已复现产品缺陷引起：物业负责人（PL）打开 `/members` 时，前端错误请求仅限服务商使用的 `/v1/providers/{id}/collector-capacity`，产生 403。候选代码已按组织类型增加守卫；同一断言在候选前端上回归 2/2 通过。
...
| 移动角色扫描 | 3/3 通过 | PC、CO 可用；SA 被移动作业壳拒绝 |
| 跨角色业务生命周期 | 1/1 通过，两次独立验证 | 第二次加强为按本次唯一批次编码核对 `¥12.34` |
| 角色菜单/真页面扫描 | 除 F-01 外通过 | 六角色所有可见菜单逐页打开，禁止菜单逐角色核验 |
...
这些目录不会提交到 Git。全量配置关闭 Playwright trace，避免原始请求头、Cookie 或令牌进入产物；诊断 JSON 仅记录脱敏后的方法、路径和状态码。

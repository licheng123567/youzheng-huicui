...
> 范围：外勤作业 App —— 催收员（CO）+ 物业协调员（PC）（BR-APP-01）；页面编号沿用 `P-APP-xx`。所有端点均为契约既有（含 §7 增量提案项，标注 ⊕）。
...
- 错误分支：401 凭据错 / 网络不可达 文案区分；非外勤角色（SA/SE/PL/VL）登入 → 提示"请使用网页端"（BR-APP-01）。注意 PL 有 `case.call` 但仍被挡。

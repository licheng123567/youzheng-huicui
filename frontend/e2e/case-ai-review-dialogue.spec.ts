...
test.describe('US-M5 AI复盘对话气泡(CO)', () => {
  async function openReview(page: any) {
    ...
    const reviewBtn = page.getByRole('button', { name: /查看并标注（AI 复盘）/ })
  ...
  test('案件作业台复盘→对话气泡 + AI 复盘小结渲染', async ({ page }) => {
    ...
    // AI 复盘小结渲染为 .bgbox（无「小结：」前缀文案）
    await expect(page.getByRole('dialog', { name: 'AI 复盘 · 本次录音' }).locator('.bgbox').first()).toBeVisible()
  ...
  test('CallRecord 独立页一致(转写文本 + AI 复盘卡片)', async ({ page }) => {
    ...
    await page.getByRole('button', { name: '获取最新通话录音' }).click()
    const detailBtn = page.getByRole('button', { name: '通话详情' })

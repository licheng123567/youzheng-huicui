import { test, expect } from '@playwright/test'
import { loginAs } from './helpers'

// v1.3.0 P2：个人中心(自助改密) + 案件搜索 真屏。
test.describe('v1.3.0 个人中心 + 搜索', () => {
  test('头部姓名→个人中心→改密对话框就位', async ({ page }) => {
    await loginAs(page, 'admin')
    // 头部姓名是 <span class="link">（非 button），点它进个人中心
    await page.locator('.top .link').filter({ hasText: '（' }).first().click()
    await expect(page).toHaveURL(/\/profile/)
    await expect(page.getByText('个人资料')).toBeVisible()
    // 「修改密码」既有 <button> 也有 <a class="btn txt">，取 button
    await page.getByRole('button', { name: '修改密码' }).click()
    await expect(page.getByRole('dialog').filter({ hasText: '修改密码' })).toBeVisible()
  })

  test('全局搜索→案件命中表就位', async ({ page }) => {
    await loginAs(page, 'admin')
    await page.goto('/search?q=' + encodeURIComponent('张'))
    await expect(page).toHaveURL(/\/search/)
    // 命中行（C-1001 张三）出现
    await expect(page.getByText('C-1001')).toBeVisible()
  })
})

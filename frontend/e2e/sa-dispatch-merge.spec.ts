...
// 撮合派单与平台公海合并（用户决定·保留一个菜单入口）。
test.describe('SA 撮合派单+平台公海合并', () => {
  test('SA 无独立「平台公海」菜单;撮合派单页有 批次派单/平台公海 两 Tab', async ({ page }) => {
    ...
    await expect(tabs.getByText('批次派单')).toBeVisible()
